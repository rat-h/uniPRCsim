	...
	def update(self,model):
		for nrn in xrange(self.number):
			...
			if (self.timetospike[nrn] - model.timetospike) < 1e-7:
				if self.process >= 2: sys.stderr.write("SPIKE\n")
				self.op[nrn] = 1
				...
				if self.process == 0 or self.process == 2:
					...
					self.periods[nrn] += (self.period_mu - self.periods[nrn]) * self.neurons[nrn][2]/self.period_tau + rnd.gauss(0,self.period_sd) * math.sqrt(self.periods[nrn]*self.neurons[nrn][2])
			else:
